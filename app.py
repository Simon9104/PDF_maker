...
# ── Colour palette (matches the invoice sample) ───────────────────────────────
C_ACCENT   = colors.HexColor("#1a5fa8")   # blue headers
C_DARK     = colors.HexColor("#1c2b3a")   # dark text
C_LIGHT_BG = colors.HexColor("#f0f4f8")   # alternating row
C_TOTAL_BG = colors.HexColor("#1a5fa8")   # "amount due" row
...
class CustomerTableApp:
    ...
    def _tab_supplier(self, nb):
        ...
        self._field(f, "ID / IČO",      self.sup_ico,     6, 0)
        self._field(f, "Tax ID / DIČ",  self.sup_dic,     7, 0)
        self._field(f, "VAT ID / IČ DPH", self.sup_icdph, 8, 0)
        ttk.Separator(f, orient="horizontal").grid(
            row=9, column=0, columnspan=4, sticky="ew", pady=8)
        self._field(f, "IBAN",          self.sup_iban,   10, 0, width=40)
        self._field(f, "SWIFT / BIC",   self.sup_swift,  11, 0)
        self._field(f, "Bank",          self.sup_bank,   12, 0)
        ttk.Separator(f, orient="horizontal").grid(
            row=13, column=0, columnspan=4, sticky="ew", pady=8)
        self._field(f, "Phone",         self.sup_phone,  14, 0)
        self._field(f, "E-mail",        self.sup_email,  15, 0)
        self._field(f, "Website",       self.sup_web,    16, 0)
    ...
    def _tab_customer(self, nb):
        ...
        self._field(f, "ID / IČO",       self.cus_ico,     6, 0)
        self._field(f, "Tax ID / DIČ",   self.cus_dic,     7, 0)
...
class PreviewWindow(tk.Toplevel):
    def __init__(self, parent, d):
        ...
        # Meta
        meta = [
            ("Issue date", d["doc_issue"]),   ("Due date", d["doc_due"]),
...
def generate_pdf(path, d):
    ...
    # ── Header: title right, number below ─────────────────────────────────
    story.append(Table(
    ...
    def sup_lines():
        items = []
        items.append(Paragraph("SUPPLIER", sSupLbl))
        items.append(Paragraph(d["sup_name"], sSupVal))
    ...
    def cus_lines():
        items = []
        items.append(Paragraph("CUSTOMER", sSupLbl))
        items.append(Paragraph(d["cus_name"], sSupVal))
    ...
    for i in range(0, len(meta_pairs), 2):
        ...
        for k, v in meta_pairs[i:i+2]:
            if k:
                row += [Paragraph(k, sMeta), Paragraph(v or "", sMetaV)]
            else:
    ...
    if cols:
        cw = W / len(cols)
        items_data = [[Paragraph(c, ps("IH", fontSize=9, textColor=colors.white,
                                        fontName="Helvetica-Bold",
                                        alignment=TA_CENTER))
                       for c in cols]]
        for row in d["rows"]:
            padded = (list(row) + [""] * len(cols))[:len(cols)]
            items_data.append([Paragraph(str(v), ps("IV", fontSize=9,
                                                      textColor=C_DARK,
                                                      alignment=TA_CENTER))
                                for v in padded])

        items_table = Table(items_data, colWidths=[cw]*len(cols), repeatRows=1,
                            style=TableStyle([
                                ("BACKGROUND",    (0,0), (-1,0),   C_ACCENT),
                                ("ROWBACKGROUNDS",(0,1), (-1,-1),
                                 [colors.white, C_LIGHT_BG]),
